...
async def test_tg(dut):
    ...
    test_num = os.getenv("TG_tb", "1") #default test 1
...
## + interfacing w/t the command line 
def run_test():
    ...
    verilog_sources = ["TG.sv"]
    runner = get_runner(sim)
    runner.build(
        verilog_sources=verilog_sources,
        hdl_toplevel="TileGrowth",
        always=True,
    )

    runner.test(hdl_toplevel="TileGrowth",
